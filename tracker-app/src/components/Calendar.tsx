...
export default function MyCalendar({ projects, customers, resources }: { projects: any[]; customers: any[]; resources: any[] }) {
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    switch (selectedCategory) {
      ...
      // case 'project':
      // return project.id === selectedCustomerId;
      case 'type':
  ...
  //   p.schedules.map((schedule: any) => {
  //     return {
  //       customer: `${p.customer.name}`,
  //       // customerId: `${p.customerId}`,
  //       p: `${p.name} - ${schedule.type} - ${schedule.status}`,
  //       type: schedule.type,
  ...
  return (
    <div style={{ height: '80vh', padding: '20px' }}>
      <div className="mb-4 flex items-center justify-end gap-4">
        <select className="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border bg-white"
